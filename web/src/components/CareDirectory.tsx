        ...
        <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
          ...
          {(searchQuery || selectedState || selectedService) && (
            <button
              ...
              className="text-xs text-emerald-600 dark:text-emerald-400 hover:underline px-2 py-1 font-semibold shrink-0"
            >
              Clear
            </button>
          )}
          ...
          <div className="sm:hidden divide-y divide-slate-100 dark:divide-slate-800/80">
            {filteredClinics.length === 0 ? (
              ...
              filteredClinics.map((clinic, idx) => (
                <div key={`${clinic.name}-${idx}`} className="p-4 space-y-2.5 hover:bg-slate-50 dark:hover:bg-slate-850/40 transition-colors">
                  ...
                  <div className="flex flex-wrap items-center gap-1.5 pt-1">
                    ...
                    {clinic.dementia_speciality ? (
                      <span className="inline-flex items-center space-x-1 px-2 py-0.5 rounded text-[11px] font-semibold bg-emerald-100 dark:bg-emerald-950 text-emerald-700 dark:text-emerald-400 border border-emerald-300 dark:border-emerald-800">
                        <CheckCircle2 className="w-3 h-3" />
                        <span>Dedicated Clinic</span>
                      </span>
                    ) : (
                      <span className="text-[11px] text-slate-400 px-2 py-0.5 rounded bg-slate-50 dark:bg-slate-800/60">
                        General Neurology
                      </span>
                    )}
                  </div>
